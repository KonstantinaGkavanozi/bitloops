    ...
    pub fn load_repo_id_for_commit(&self, commit_sha: &str) -> Result<String> {
        ...
        self.sqlite.with_connection(|conn| {
            ...
            let repo_id: String = stmt
                ...
                .with_context(|| {
                    format!(
                        "no production artefacts found for commit {}; materialise production artefacts first (use `cycloops devql tasks enqueue --kind ingest --status` or `cycloops devql tasks enqueue --kind sync --status`)",
                        commit_sha
                    )
                })?;
            Ok(repo_id)
...
#[cfg(test)]
#[path = "sqlite_relational/tests.rs"]
mod tests;
